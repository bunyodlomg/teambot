...
const tasksSend = async (ctx, u) => {
    ...
    let us = [];
    u.forEach(user =>{
        us.push({user_id:user,status:"SENDED",sended:new Date()})
    })
    ...
    const tas = await tasksData.save()

    for (let i = 0; i < us.length; i++) {
        await ctx.api.sendMessage(us[i].user_id, 'Sizga yangi vazifa berildi!\nVazifa : ' + task.task,{
            reply_markup: {
                inline_keyboard: [
                    [
                        {
                            text: 'Qabul qilish',
                            callback_data: "accept_task_" + us[i].user_id + "_" + tas._id,
                        }
                    ]
                ],
